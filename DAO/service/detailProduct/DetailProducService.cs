        ...
        public List<DetailProduct> findAll()
        {
            ...
            try
            {
                ...
                foreach (DataRow row in dataTable.Rows)
                {
                    ...
                    int quantity = Convert.ToInt32(row[6]);
                    bool status = Convert.ToInt32(row[7]) > 0;
                    listProduct.Add(new DetailProduct(productCode, productCode, size, color, priceIn, priceOut, quantity, status));
                }
            }
            catch (Exception)
            {
        ...
        public List<DetailProduct> findAllByProduct(string idProduct)
        {
            ...
            try
            {
                ...
                foreach (DataRow row in dataTable.Rows)
                {
                    ...
                    int quantity = Convert.ToInt32(row[6]);
                    bool status = Convert.ToInt32(row[7]) > 0;
                    listProduct.Add(new DetailProduct(idProduct, productCode, size, color, priceIn, priceOut, quantity, status));
                }
            }
            catch (Exception)
            {
